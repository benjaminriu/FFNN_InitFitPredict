# THIS IS AN IMPLEMENTATION OF FEED-FORWARD NEURAL NETWORKS FOLLOWING THE SCIKIT-LEARN API.
# IT INCLUDES BOTH REGULAR TRAINING SCHEMES AND ADACAP.
# DEFAULT ARCHITECTURE IS A MLP COMPATIBLE WITH TABULAR DATASETS BUT X CAN BE A NUMPY TENSOR OF ANY SHAPE (n, ...) AS LONG AS THE DIMENSIONS MATCH THE CHOSEN ARCHITECTURE INPUT SHAPE. SEE DOCUMENTATION FOR AN EXAMPLE USING A CONVNET ON MNIST.

if True:  # For code folding
    # For a NVIDIA 2080Ti (11.G of VRAM):
    # Max width of any matrix; scales quadraticaly with VRAM
    ...
    # torch lr schedulers which should be called at each iteration, not each epoch (not exhaustive!)
    ITERSCHEDULERS = ["OneCycleLR", "CyclicLR", "ReduceLROnPlateau"]
    # replace total steps argument with epochs * n_batches
    STEPSSCEDULERS = ["OneCycleLR", "CyclicLR"]
    ...
    def n2f(array):
        ...
        if type(array) == float:
            ...
            return np.array(array).reshape(-1)[0]
    ...
    # see _initialize_path, _save_weights and _load_weights
    REP, PATH = "./", "tempory_model"
    GRID_START, GRID_END, GRID_SIZE = 1e1, 1e7, 101  # see _gridsearch_closeform_param
    # see _stratify_continuous_target (CPU memory and runtime safeguards)
    MAX_TRAINING_SAMPLES, MAX_TREE_SIZE = int(1e4), 50
    from sklearn.preprocessing import LabelEncoder
    from scipy.special import softmax as softmax_func
    from scipy.special import logit as logit_func


class FeedForwardNeuralNetwork(BaseEstimator, metaclass=ABCMeta):
    @abstractmethod
    def __init__(self,  # see subclasses for default values
                 ...
                 # iterations, not epochs (epochs = max_iter/batch_size), int
                 max_iter,
        ...
        self.release_train_memory = release_train_memory
    ...
    def initialize_model(self, X, y):
        ...
        if self.validation:
            X, self.X_valid, y, self.y_valid = self._initialize_valid(X, y)
        self._initialize_training(*X.shape)
        datas = self._initialize_data(X, y)
        if self.use_closeform:
            self._initialize_closeform(datas)
    ...
    def _initialize_state(self):
        ...
        self.torch_random_state = torch.Generator(device=device).manual_seed(
            int(self._random_state.uniform(0, 2**31)))
        ...
        self.use_closeform = self.adacap and self.closeform_parameter_init not in [
            0, 0., False, None]
        self.add_permut_term = self.use_closeform and self.n_permut not in [
            False, None, 0, 0.] and type(self.permutation_scale) not in [type(False), type(None)]
        self.diff_permut_term = self.add_permut_term and self.permutation_scale not in [
            False, None, 0, 0.]
        self.add_target_rotation = self.target_rotation_scale not in [
            False, None, 0, 0.]
    ...
    def _initialize_path(self):
        ...
        while self.PATH in os.listdir(self.save_repository):
            path_index += 1
            self.PATH = PATH+str(path_index)+".pt"
        # create empty file in case two models start training simultaneously
        open(self.save_repository+self.PATH, 'a').close()
    ...
    def _initialize_task(self, y):
        ...
        if self.multi_class:
            ...
            static_intercept = y.mean() / \
                target_std if self.center_target else 0.
    ...
    def _initialize_training(self, data_length, n_features, *args):
        self.batch_learning = type(self.batch_size) in [float, int] or (
            data_length > MAX_GPU_MATRIX_WIDTH and device == "cuda")
        if type(self.batch_size) in [float] and not self.batch_learning:
            if self.batch_size * data_length > MAX_GPU_MATRIX_WIDTH and device == "cuda":
                self.batch_learning = True
        if type(self.batch_size) in [int] and not self.batch_learning:
            if self.batch_size > MAX_GPU_MATRIX_WIDTH and device == "cuda":
                self.batch_learning = True
        ...
        self.loss_func = getattr(torch.nn, self.loss_function)(
            reduction='none', **loss_params)
        self.torch_seed = int(self._random_state.uniform(0, 2**31))
        if type(self.hidden_nn) in [bool]:
            self.hidden_nn = self._get_default_hidden_nn()
            self.hidden_params.update(
                self._get_default_hidden_params(n_features))

        if device != "cpu":
            # Otherwise weights initialized without fix seed
            torch.cuda.manual_seed_all(self.torch_seed)
            self.hidden_layers = self.hidden_nn(**self.hidden_params)
            # Otherwise dropout not deterministic
        ...
        self.optimizer_instance = getattr(torch.optim, self.optimizer)(
            lr=self.learning_rate, params=params, **self.optimizer_params)
        self.cst_lr = self.lr_scheduler in [None, False]
        if not self.cst_lr:
            self.update_lr_every_step = (
                self.batch_learning == False) or self.lr_scheduler in ITERSCHEDULERS
            ...
            self.lr_scheduler_instance = getattr(torch.optim.lr_scheduler, self.lr_scheduler)(
                self.optimizer_instance, **updated_lr_scheduler_params)
        del params
    ...
    def _initialize_valid(self, X, y):
        ...
        def roc_with_proba_ovr(y, probas): return roc_auc_score(
            y, probas, multi_class="ovr")
        ...
        self.valid_func = self.predict_proba if is_classifier(
            self) else self.predict
        ...
        if self.should_stratify:
            ...
            stratify = None
        valid_size = int(self.validation_fraction * len(y)) if type(
            self.validation_fraction) == type(0.2) else self.validation_fraction
    ...
    def _stratify_continuous_target(self, y):
        ...
        MAX_TRAINING_SAMPLES, MAX_TREE_SIZE = int(
            1e4), 50  # CPU memory and runtime safeguards
    ...
    def _initialize_batch(self, n_samples):
        if type(self.batch_size) == float:
            batch_length = min(MAX_GPU_MATRIX_WIDTH,
                               int(n_samples * self.batch_size))
    ...
    def _initialize_data(self, X, y):
        ...
        if self.batch_learning:
            ...
            if self.add_permut_term:
                datas["target_permuted"] = self._cast(n2t(datas["y_permuted"]))
        return datas
    ...
    def _update_data(self, datas):
        if self.batch_learning:
            ...
            if self.add_permut_term:
                datas["target_permuted"] = self._cast(
    ...
    def _generate_batch(self, n_samples):
        shuffled_indexes = shuffle(
            np.arange(n_samples), random_state=self._random_state)
    ...
    def _gridsearch_closeform_param(self, datas):
        candidates, losses = np.geomspace(
            GRID_START, GRID_END, GRID_SIZE), np.zeros(GRID_SIZE)
        with torch.no_grad():
            activation = self._forward_pass(datas["input"])
            if self.closeform_intercept:
                activation = torch.cat([activation, torch.ones(
                    [activation.shape[0], 1], device=device)], dim=-1)
            ...
            for i, candidate in enumerate(candidates):
                inv_mat_dot_activation = self._get_inv_mat(
                    activation, n2t(np.log(candidate))) @ activation.T
                ...
                if self.diff_permut_term:
                    ...
                    loss += self.permutation_scale * \
                        self._compute_MLR_penalty(pred_permut, target_permuted)
    ...
    def _reduce_loss(self, point_wise_loss):
        if self.loss_function == "MSELoss":
            ...
            return point_wise_loss.mean(dim=-1)
    ...
    def _get_inv_mat(self, activation, ridge_coef):
        ...
        while not inversion_successful and inversion_attempts < MAX_ATTEMPTS:
            try:
                diag_mat = torch.diag(torch.ones(
                    activation.shape[1], device=device) * torch.exp(ridge_coef))
                ...
                # multiply diag by 10 until inversion works
                ridge_coef += n2t(np.log(10.))
        ...
        if inversion_successful:
            ...
            # In that case pred = X @ X.T @ Y
            return torch.eye(correl_mat.shape, device=device)
    ...
    def _format_target(self, target):
        if self.multi_class:
            target = torch.transpose(torch.nn.functional.one_hot(
                target, num_classes=self.n_classes), -1, -2)
    ...
    def _rotate_pred(self, target, projector):
        if self.add_target_rotation:
            epsilon = torch.normal(0., self.target_rotation_scale, size=target.shape,
                                   generator=self.torch_random_state, device=device)
            target = (target + target @ projector) / \
                2 + epsilon - epsilon @ projector
        return target

    def _dither_target(self, target):
        if self.add_dithering:
            target = target + torch.normal(0., self.dithering_scale, size=target.shape,
                                           generator=self.torch_random_state, device=device)
        return target
    ...
    def _forward_propagate(self, datas):
        ...
        if self.use_closeform:
            self.record["lambda"].append(
                np.exp(n2f(t2n(self.closeform_param))))
            formated_target = self._format_target(target)
            if self.closeform_intercept:
                activation = torch.cat([activation, torch.ones(
                    [activation.shape[0], 1], device=device)], dim=-1)
        ...
        if self.add_permut_term:
            with no_context()() if self.diff_permut_term else torch.no_grad():
                ...
                formated_target_permutation = self._format_target(
                    target_permutation)
                if self.add_target_rotation:
                    pred_permutation = self._rotate_pred(
                        formated_target_permutation, projector)
                else:
                    pred_permutation = formated_target_permutation @ projector
                permut_loss = self._compute_MLR_penalty(
        ...
        if self.print_record:
            ...
            if self.current_iter % int(self.verbose) == 0:
                print("| "+f2s(str(self.current_iter)), *
                      map(lambda value: f2s(value[-1]), self.record.values()))
        return loss
    ...
    def _save_weights(self):
        ...
        if self.current_iter == self.best_iter:
            ...
            if self.use_closeform:
                self.saved_output_weights = torch.clone(self.output_weights)
    ...
    def _release_train_memory(self):
        ...
        if self.use_closeform:
            del self.closeform_param
        del self.optimizer_instance
        if not self.cst_lr:
            del self.lr_scheduler_instance
    ...
    def _forward_pass_fast(self, activation):
        with torch.no_grad():
            ...
            if self.use_closeform:
                if self.closeform_intercept:
                    hidden_output = torch.cat([hidden_output, torch.ones(
                        [hidden_output.shape[0], 1], device=device)], dim=-1)
    ...
    def _get_default_hidden_params(self, n_features):
        ...
        output = 0 if self.use_closeform else true_output
        return {"output": output, "n_features": n_features}


class FeedForwardRegressor(RegressorMixin, FeedForwardNeuralNetwork):
    def __init__(self,
                 hidden_nn=False,
                 hidden_params={},
                 optimizer="Adam",
        ...
        super().__init__(
            ...
            adacap=adacap, closeform_parameter_init=closeform_parameter_init, closeform_intercept=closeform_intercept, n_permut=n_permut, permutation_scale=permutation_scale, dithering_scale=dithering_scale,
            ...
            random_state=random_state, release_train_memory=release_train_memory, save_repository=save_repository, verbose=verbose)

    def predict(self, X):
        return self._predict_hidden(X)


class FeedForwardClassifier(ClassifierMixin, FeedForwardNeuralNetwork):
    def __init__(self,
                 hidden_nn=False,
                 hidden_params={},
                 optimizer="Adam",
...
# BASIC MLP EXAMPLE TO ENSURE THIS FILE IS SELF-CONTAINED
if True:
    import torch
    ...
    class DefaultDenseLayers(torch.nn.Module):  # Basic Feed Forward Dense MLP
        def __init__(self,
                     n_features,
